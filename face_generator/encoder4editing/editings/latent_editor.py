...
class LatentEditor(object):
    ...
    # custom version of apply_ganspace that resturns latents instead of images
    def apply_ganspace_custom(self, latent, ganspace_pca, edit_directions):
        edit_latents = ganspace.edit(latent, ganspace_pca, edit_directions)
        return edit_latents
    ...
    # custom version of _latents_to_image that is not a private function
    def latents_to_image_custom(self, latents):
        with torch.no_grad():
            images, _ = self.generator([latents], randomize_noise=False, input_is_latent=True)
            if self.is_cars:
                images = images[:, :, 64:448, :]  # 512x512 -> 384x512
