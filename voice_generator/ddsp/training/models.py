...
class Model(tf.keras.Model):
  ...
  def restore(self, checkpoint_path):
    ...
    if latest_checkpoint is not None:
      ...
      print('Loading model took ', time.time() - start_time, ' seconds')
    else:
      print('Could not find checkpoint to load at %s, skipping.',
                   checkpoint_path)
...
class Autoencoder(Model):
  ...
  def decode(self, conditioning, training=True):
    ...
    # amps = np.load('/media/gabriel/Extra/SYNTHETIC/voice_generator/amps.npy')
    # amps = np.resize(amps, processor_inputs['amps'].shape)
    # amps = tf.convert_to_tensor(amps, dtype=tf.float32)
    # processor_inputs['amps'] = amps

    # harm = np.load('/media/gabriel/Extra/SYNTHETIC/voice_generator/harm_dist.npy')
    # harm = np.resize(harm, processor_inputs['harmonic_distribution'].shape)
    # harm = tf.convert_to_tensor(harm, dtype=tf.float32)
    # processor_inputs['harmonic_distribution'] = harm

    # noise = np.load('/media/gabriel/Extra/SYNTHETIC/voice_generator/noise_mag.npy')
    # noise = np.resize(noise, processor_inputs['noise_magnitudes'].shape)
    # noise = tf.convert_to_tensor(noise, dtype=tf.float32)
    # processor_inputs['noise_magnitudes'] = noise
