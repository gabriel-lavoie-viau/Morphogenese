...
class MfccTimeDistributedRnnEncoder(Encoder):
  ...
  def compute_z(self, conditioning):
    if conditioning['audio'] == None:
      mfccs = conditioning['mfccs']       # ---INSERT MFCC DATA HERE--- #
    else:
    ...
    # for z in mfccs.numpy():
    #   for y in range(z.shape[1]):
    #     filename = "./audio_features/mfccs_" + str(y) + ".txt"
    #     with open(filename, 'w') as f:
    #       for x in range(z.shape[0]):
    #         item = z[x][y]
    #         f.write("%s " % item)

    # with open('./audio_features/mfccs_python.txt', 'w') as f:
    #     for item in mfccs.numpy():
    #         f.write("%s" % item)
